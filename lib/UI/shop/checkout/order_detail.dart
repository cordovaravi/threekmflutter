...
class _OrderDetailsState extends State<OrderDetails> {
  ...
  formatDate(dateUtc) {
    ...
    var utcDate = dateFormat.format(dateUtc); // pass the UTC time here
  ...
  Widget build(BuildContext context) {
    ...
    var sum = 0.0;
    Box orderStatusBox = Hive.box('orderStatusBox');
    ...
    return WillPopScope(
      onWillPop: () async {
        ...
        Box orderStatusBox = Hive.box('orderStatusBox');
        orderbox.clear();
        log('${orderStatusBox.get('orderDates')} ${formatDate(DateTime.now())} ${orderStatusBox.get('orderDates') == formatDate(DateTime.now()).split('-')[1]}========');
        log('${orderStatusBox.get('orderStatus') != 'delivered'}');
        if (orderStatusBox.get('orderDates') ==
                formatDate(DateTime.now()).split('-')[1] &&
            orderStatusBox.get('orderStatus') != 'delivered') {
          orderStatusBox.put('orderId', data?.projectId);
          orderStatusBox.put('orderDate', data?.centerTime);
          ShowOrderStaus();
        }
                                ...
                                builder: (_, snapshot) {
                                  if (snapshot.data != null) {
                                    OrderRealtimeDetailModel orderData =
                                        snapshot.data
                                            as OrderRealtimeDetailModel;
                                    orderStatusBox.put(
                                        'orderDates', orderData.date);
                                    orderStatusBox.put(
                                                                ...
                                                                Center(
                                                                  child:
                                                                      ElevatedButton(
                                                                          onPressed:
                                                                              () {
                                                                            launch('${orderData.deliveryDetails.trackingUrl}');
                                                                          },
                                                                          child: Text(
                                                                              'Track your order'),
                                                                          style: ButtonStyle(
                                                                    ...
                                                                    Padding(
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        child:
                                                                            Image(
                                                                          image:
                                                                              NetworkImage('${orderData.restaurantCover}'),
                                                                          width:
                                                                              100,
                                                                          height:
                                                                              60,
                                                              ...
                                                              child: Column(
                                                                children: [
                                                                  ListView.builder(
                                                                      ...
                                                                      shrinkWrap: true,
                                                                      itemCount: orderData.deliveryLogs.length,
                                                                      itemBuilder: (_, i) {
                                                                        return TimelineTile(
                                                                          nodeAlign:
